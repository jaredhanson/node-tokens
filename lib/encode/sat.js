...
module.exports = function(options, keying) {
  ...
  return function sat(claims, options, cb) {
    console.log('STRUCTURED ACCESS TOKEN ENCODE');
    console.log(claims);
    console.log(options);
    ...
    var query  = {
      ...
      //encryptionAlgorithms: options.encryptionAlgorithms
    }
    ...
    keying(query, function(err, keys) {
      ...
      var header = { typ: 'JWT', alg: 'RS256', kid: key.id }
        , token;
      try {
        token = jws.sign({ header: header, payload: claims, secret: key.privateKey });
      } catch (ex) {
        return cb(ex);
      }
