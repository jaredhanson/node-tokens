...
module.exports = function(options, keying) {
  ...
  return function sat(claims, options, cb) {
    ...
    // TODO: Check that audience is only a single value.  That's all JWT compact
    // format can handle
    ...
    keying(query, function(err, keys) {
      ...
      if (!key.algorithm) {
        if (key.secret) { key.algorithm = 'hmac-sha256' }
        else { key.algorithm = 'rsa-sha256' };
      }
