...
module.exports = function(options, keying) {
  ...
  return function jwt(claims, key, options, cb) {
    ...
    function keyed(err, key) {
      ...
      // TODO: Consider better way to pass key algorithm option
      key.algorithm = options.algorithm || key.algorithm || (confidential ? 'aes128-cbc-hmac-sha256' : (key.secret ? 'hmac-sha256' : 'rsa-sha256'));
      ...
      // TODO: Fix options argument
      //if (options.id) { header.kid = options.id; }
