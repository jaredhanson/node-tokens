...
module.exports = function(options, keying) {
  ...
  return function jwt(token, parameters, cb) {
    ...
    if (parsed.type == 'JWS') {
      ...
      issuer = decoded.payload.iss || issuer;
    }
    ...
    // TODO: Check if issuer in token matches parameters, if not, reject
    
    keying(parameters.issuer || issuer, query, function(err, key) {
      if (parsed.type == 'JWS') {
        _jws.verify(token, decoded.header, decoded.payload, key.secret || key.privateKey, function(err, claims, conditions) {
          if (err) { return cb(err); }
          // TODO: Pass the issuer back, if loaded during keying?
